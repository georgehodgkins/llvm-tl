#include <llvm/Analysis/TLProp/FuncModel.hpp>
#include <cassert>
#include <sstream>
#include <algorithm>
...
static void addTransitions (const BasicBlock* from) {
	...
	for (unsigned i = 0; i < arnold->getNumSuccessors(); ++i) {	
		...
		trans.addAction(SyncAssignment(assgn));
		addTransition(trans);
...
FunctionGAL::FunctionGAL (const Function* F) : GAL(F->getName()) {
	...
	std::for_each(F->begin(), F->end(), addTransitions);
